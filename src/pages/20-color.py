...
PAGE = {
 ...
 "blocks": [
  ...
  ("p", "상승이 빨강, 하락이 파랑이다. HIG 도 이걸 지지한다 — 색 문서가 주식 앱 스크린샷 두 장으로 <b>영어권은 초록이 상승, 중국어권은 빨강이 상승</b>이라고 직접 예시한다. 색의 뜻은 문화가 정한다는 것이 원래 규정이다."),
  ...
  ("raw", spec(["대상", "최소 대비", "검사"], [
   ...
   ], "기본값이 이를 못 맞추면 <b>최소한 고대비 모드에서는</b> 맞춘다 — HIG 가 명시한 방식이고, 원전 자신도 시스템 색에 별도 accessible 변형을 둔다. 우리 <code>@media (prefers-contrast: more)</code> 블록이 그 역할이다.")),
 ],
}
