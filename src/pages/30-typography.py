...
scale_fig = fig('<div style="display:flex;flex-direction:column;gap:var(--sp-3)">' + "".join(
    ...
    f'{S("고요한 정밀 Calm Precision", f, "--fw-sb", t)}</div>' for f, t in steps) + '</div>',
    "이 열두 단계 안에서만 고른다.")
...
PAGE = {
 ...
 "blocks": [
  ...
  ("raw", scale_fig),
  ("p", "각 단계는 HIG 텍스트 스타일에 대응한다. 우리가 임의로 만든 숫자가 아니다."),
  ("raw", spec(["토큰", "px", "HIG 스타일 (iOS Large)", "쓰는 곳"], [
  ...
  ("rule", "이 하나가 '고요한 정밀이 나오는가' 를 가장 크게 좌우한다",
  ...
  ("p", "HIG 실측은 본문 17/22 = 1.29다. 그 값은 라틴 문자 기준이다. 한글은 받침 때문에 글자 하나의 세로 밀도가 높아 같은 비율이면 줄이 서로 붙어 보인다."),
  ("raw", cmp2([
     ("do", lh_ko,    "<code>--lh-base</code> 1.55 — 한글 본문"),
     ("no", lh_apple, "1.29 — 라틴 기준 그대로"),
   ], "본문만 넓힌다. 제목과 수치는 HIG 비율(<code>--lh-title</code> 1.21, <code>--lh-head</code> 1.29)을 그대로 쓴다 — 큰 글자는 원래 행간이 좁아야 덩어리로 읽힌다.")),
